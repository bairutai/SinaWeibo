...
public class User {
	...
	public static  int parseflower(String jsonObjectStr, WeiboApplication app) throws JSONException{
		...
		JSONArray jsonArray=jsonObject_flower.getJSONArray("users");
		for(int i=0;i<jsonArray.length();i++) {
			JSONObject jsonObject = (JSONObject)jsonArray.get(i);
			User user = new User();
			user.id                 = jsonObject.optString("id", "");
			user.idstr              = jsonObject.optString("idstr", "");
			System.out.println(user.idstr);
			user.screen_name        = jsonObject.optString("screen_name", "");
			user.name               = jsonObject.optString("name", "");
			user.province           = jsonObject.optInt("province", -1);
			user.city               = jsonObject.optInt("city", -1);
			...
			user.bi_followers_count = jsonObject.optInt("bi_followers_count", 0);
			user.lang               = jsonObject.optString("lang", "");       
			user.previous_cursor = jsonObject.optInt("previous_cursor");
			user.next_cursor = jsonObject.optInt("next_cursor");
			user.total_number  = jsonObject.optInt("total_number");
